...
| IT-6 | Big-read detection **(user)** | `python3 -c "print('x'*200000)" > /tmp/goldie-big.txt`. User asks a Cursor agent: "Read /tmp/goldie-big.txt and tell me its length." Then take a snapshot. | That chat has `bloatTokens` ≥ 10000, and `bloatLabel` names the **largest** single result: `goldie-big.txt`, or "output of `…`" if a bigger shell output of the same content happened earlier (that one really is re-sent every step). |
| IT-7 | Suggestions → Copy handoff **(user)** | Temporarily set `{"heavyRatio": 1.2, "alarmedRatio": 2, "speechCooldownMinutes": 1}` in the config, then restart Goldie. Wait for the orange number badge on the bowl. Click Goldie → **Goldie's suggestions** → **Copy handoff**. Then `pbpaste \| head -5` (report only the section headers). | The clipboard holds a handoff starting with "I'm continuing work…" and `## Goal`. Cursor is **not** opened or focused, and no files appear under the repo's `.goldie/`. |
...
| IT-14 | Loop guard **(user)** | Menu bar → turn on **Loop guard**, then restart Cursor. User asks a Cursor agent: "Run `echo goldie-loop-test` six times, as six separate terminal commands, without editing any files." | From the 5th run on, Cursor shows Goldie's block message, the agent is told to change approach, and Goldie shows a "stopped a wasted step" toast. The chat keeps working. |
| IT-15 | Big-read guard **(user)** | Turn on **Big-read guard**, then restart Cursor. `python3 -c "print('x'*400000)" > /tmp/goldie-huge.txt`. User asks the agent: "Read /tmp/goldie-huge.txt in full." | The first read is refused with Goldie's message. If the agent asks again, it's allowed. Turn both guards off afterwards unless the user wants them. |
